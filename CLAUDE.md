...
The résumé is rendered once per variant defined in `src/_data/variants.json`. There is no separate "default" résumé — the variant with `permalink: "/"` serves at the site root; other variants deploy at `/cvpage/<permalink>`.
